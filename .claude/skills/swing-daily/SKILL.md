...
2. **見落とされているリスク**の指摘（断片間の矛盾、開示の新規リスク記載、
...
銘柄ごとに `proceed` / `skip` を決める。Step 3.6 を実施した銘柄は、反証ポイントへの
応答を済ませてから確定する。
...
- Step 3.6 で検討したリスクを `reasons[].text` に反映してよい。ただし根拠が定性テキスト
  由来なら該当 `source_ids` を必ず付け（AC6・AC7・AC10）、入力に無い情報を足さない（AC8）。
  CON-03 の言語規律（断定的売買指示・命令形の禁止、hedge 表現）は従来どおり厳守する
  （AC3〜AC5・AC12）
...
平日の定時に launchd 等から `claude -p "/swing-daily"` として**無人起動**される運用が
ある（スケジュール自体はこのスキルの外で管理される）。対話セッションでは従来どおり
...
- 反証パス（Step 3.6）の出力をファイル化して `analysis_work/` や
  `analysis_result.json` に混ぜること（反証は verdict の材料であり出力契約ではない）
