...
1. 波を起動する直前に、その波の締切を測るウォッチャを 1 本だけ背景実行する。
   ...
   end=$(( $(date +%s) + 900 )); while [ "$(date +%s)" -lt "$end" ]; do sleep 30; done; echo TIMEBOX_REACHED
   ```

   `run_in_background` で起動し、完了通知をその波の締切とみなす。起動した各
   エージェントの ID／名前は `TaskStop` に渡すため必ず控える
...
4. ウォッチャを起動できない場合（承認待ちになる、コマンドが使えない）は**再試行せず**、
   時計に頼らない代替規則へ切り替える: その波の**過半が完了した**時点で、残っている
