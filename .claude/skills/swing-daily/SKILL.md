...
#372）。プリフェッチが空だった・例外が発生した・最新の bar がまだ引けていない
セッションのものだった、のいずれか。この場合:
...
**`PREFLIGHT_ABORT[price_fetch_failed]`** — 価格データの取得そのものが例外で
失敗し、引けた取引日を判定できなかったことを意味する（Issue #376）。上記
