...
1. 波を起動する直前に、その波の締切を測るウォッチャを 1 本だけ背景実行する。
   ...
   **起動できたことを確認してから波へ進む。** この形は相対パスなので、シェルの
   ...
   （`No such file or directory`）で即座に落ちる。2026-08-19 の dry-run 2 本が
   実際にこれで落ちており、待つための唯一のブロッキング手段が無いまま先へ進んでいた。
...
- **起動中のサブエージェントが残っている間は、ツール呼び出しを含まないターンを
  ...
  締切ウォッチャの完了待ち、または未完了エージェントの `TaskStop`。ブロックする手段が
  1 つも使えないなら待たず、完了ベースの代替規則（「親（統括セッション）の待ち方」の
  4）へ切り替えて**同じターンのうちに**先へ進む
