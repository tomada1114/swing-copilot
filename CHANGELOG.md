...
- `has_mixed_basis_signature`/`first_mixed_basis_jump`（Issue #421）が誤検知する
  ...
  そのほとんどは分割の時期を無視して銘柄の全分割 factor を候補に入れていたことが
  原因の誤検知だった（例: AIG 2008-09-16、KEY 2008-09-29 は、一致する factor の
  分割がいずれも run より前の年代にしか無い）。`_first_reversing_flip` が見つける
  逆ジャンプ対 `(i, j)` に対し、対を説明できる分割を `split.ex_date > dates[j - 1]`
  （run の最終行より**後**、厳密不等号）を満たすものだけに絞り、さらに run の長さ
  （`j - i`）が新定数 `_MAX_FLIP_RUN_SESSIONS = 25` セッション以下であることを要求
  するようにした（Issue #421 の唯一の既知の欠陥である MNST は run 長 1〜3 セッション
  だった一方、月〜年単位で水準が保たれる run は市場が実際にその値を付けた暴落と戻り
  である）。あわせて、対の2本のジャンプが**同一の**分割の `{factor, 1/factor}` に
  一致することも要求する（異なる2分割の factor をまたいだ偶然の対を排除する）。
  誤検知確認済みの3銘柄（AIG 2008-09-16、KEY 2008-09-29、ORCL 1990-03-28）はいずれも
  解消し、19 件の誤検知は2件（JKHY 1990-06-06、WDC 2002-07-22）まで減った。残る2件は
  factor が小さく実勢の往復と基準の反転が算術的に区別できないという既知の限界
  （PR #422）で、本 Issue のスコープ外として明記して残す（ホワイトリスト化や、
  ジャンプ日に分割記録が無ければ実勢変動とする案は、前者は誤検知の再発を隠し、
  後者は真陽性を全滅させるため、どちらも採らなかった）。両関数は第1引数を
  `close` の `Series` から `date`/`close` 列を持つ `DataFrame` に変更した（非公開
  API のため破壊的変更ではない）。追加した2条件はどちらも判定を単調に狭くする
  合接なので、既存の「異常なし」判定が新たに検出へ変わることは原理的に起こらず、
  510 銘柄の再スキャンは不要である
...
- `copilot-backfill rebuild` が 510 銘柄中 146 銘柄を拒否して Issue #413 の修復手順が
  ...
  36 年の日足では通常のボラティリティで頻繁に成立する（実測 510 銘柄中 153 銘柄が該当。
  分割が 1 件も無い `^VIX`/`^TNX`/`COIN`/`META`/`MRNA`/`PLTR` を含み、これらは
  `raw = close` が自明に正解なのに構造的に必ず拒否されていた）。あわせて MNST の実応答
  を調べ直したところ、Yahoo の壊れ方は行ごとにばらばらではなく、調整済み `close` は
  2005/2006/2012/2016/2023 の 5 分割すべての境界で連続し（比 1.005〜1.102）、段差が
  出るのは 2026-08-11 だけ（比 1.985）——**最新の 1 分割だけを履歴全体へ伝播し損ね、
  その ex_date 前 3 週間の 5 セッションにだけ patchy に適用している**。そこで
  `unadjust_yahoo_bars` を「行ごとの 2 仮説探索」から「どの分割が伝播済みか」という
  1 パラメータの推定へ組み替えた: 分割ごとに ex_date 境界で伝播判定し（複数の ex 前
  セッションが投票するので基準の違う 1 行では答えが動かない）、全て伝播済みなら
  `close x cum` を返し、未伝播の分割があるときだけその分割が動かすはずの行を最新バー
  から後ろ向きに分類する。分類は**生値ドメインではなく Yahoo の調整済みドメイン**で
  行う（生値は分割のたびに必ず段差になるので後ろ向きの歩みが掴む手がかりが無い）。
  境界がどちらとも読めない分割——逆分割やスピンオフの経済的な値動きが機械的な値動きを
  覆い隠す AIG 2009 / EXPE 2011 / JCI 2007 / TMUS 2013 / VTR 2015——は証拠が無いこと
  を理由に拒否せず、Issue #413 以前と同じく Yahoo の契約どおり扱う。拒否は「未伝播の
  分割があり、分類後も説明できない行が残る」という積極的な証拠があるときだけ。
  `has_mixed_basis_signature`/`first_mixed_basis_jump` は splits を引数に取り、逆
  ジャンプ対の各比が factor（または 1/factor）に一致することを要求する（許容 6%。
  基準の反転は正確な算術なので必要な緩みは同日の実勢リターン分だけでよく、MNST の
  実応答にある 6 回の反転はすべて 4.1% 以内）。`write_bars` の書き込みゲートは分割を
  `corporate_actions` から短く 1 回読み、`copilot-backfill check` は DuckDB を
  **read-only** で開く（書き込み接続は open 時にテーブルを ensure するので、「何も
  書き込まない」監査が何かを書いてしまう）。日次 run の短い窓は不変で、400 日窓・
  770 日窓の MNST は全履歴と同一の as-traded 系列を返す。ストア全 510 銘柄を 1990 年
  以降で取り直して通すと 507 銘柄すべてが解決し拒否は 0、MNST は
  `07-29 = 97.23`/`08-03 = 93.55` と仮想台帳の `entry_price` に一致する。MNST の実応答
  を 2 つのゴールデンフィクスチャとしてテストに残した
...
- `technical_signals.pullback.band_atr_multiple` を `null` から `2.0` へ変更し、
  プルバック帯の判定を固定割合から ATR14 単位へ切り替えた。
