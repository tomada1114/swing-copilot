...
| 層 | 実体 | 役割 | 主な利用者 |
|---|---|---|---|
| ① Parquet | `data/bars/`（`year=YYYY`パーティション） | 日足時系列の生（as-traded、企業行動未調整）バーを不変に列指向で永続化。追記・大量読み出しに強い。形式マーカー`_format.json`（`basis=raw, version=2`）で保存基準を明示し、未移行ストアへの読み書きはfail-fastする（`copilot-backfill rebuild`が正規化する）。 | DataProvider（書き込み）、DuckDB（読み出し元） |
| ② DuckDB | `data/copilot.duckdb` | Parquetへのビュー、ファンダメンタルズ、企業行動（`corporate_actions`: 分割・配当イベント）、ユニバース履歴、実行/ステップ、候補、落選理由、リスク評価、収集テキスト、verdict/仮想ポジションを保持する唯一の構造化ストア。 | screening/*, risk/*, tracking/*, report/*, pipeline/*, backtest/* |
...
- **`reports/`を同期対象に加えた直後は、一度だけ運用者側からの初期投入pushが必要である**（Issue #370）。温存ガードが効くのはリモートが当該ルートのキーを1件も持たない間だけなので、先に定時実行がpushしてしまうとリモートには「その日の1run」だけが載った状態で温存が解除され、次の`just data-pull`が運用者手元の過去アーカイブをミラー削除する。マージ後・次の定時実行前に、手元の`reports/`を含めてpushしておくこと
...
- `reports/<run_date>/<run_id>/`は単調増加し上限が無い（年換算で約8,000オブジェクト・約520MB）。バイト数（74%が`analysis_input.json`単体）より先に効くのは、無人実行のたびにまっさらなランナーが払う**オブジェクト数**（逐次GETの回数）である。そこで`pull --reports-window N`は、manifestのキーから`<run_date>`を集め、**新しい順にN個のrun dateに属する`reports/`キーだけ**を取得する（`data/`は常に全件）。カレンダー日数ではなくrun dateの個数で数えるため、休場や実行漏れがあっても必要な日数は確実に入る。CI専用のオプションであり、`justfile`の`data-pull`・運用者のローカル`pull`は常に全件のままである（Issue #373）。既定値は`N=10`——スキルの日跨ぎ開示断片流用が必要とする最大2営業日に対し、`copilot-retro collect`の取りこぼし回収の猶予を見込んだ値
    - 窓は`data/.r2_sync_state.json`の`reports_window`フィールドに記録される。CLIフラグではなく**この記録済みの窓**から`push`側の挙動を導出するため、フラグの付け忘れが事故にならない
    - `push`のガベージコレクションは、記録された窓がある間`reports/`プレフィックス全体で完全に抑止される。窓外のキーは（このworking copyが一度も取得していないだけで）manifestからは引き続き参照され続け、削除もされない
    - `--reports-append-only`のガードは「ローカルに存在するキーの内容が一致すること」だけを見る。ローカルに無いキーは、窓の外（GC抑止で保護される）である限り違反として扱わない。窓の中で欠けている・書き換わっているキーは従来どおり違反である
    - `_guard_against_emptying_a_populated_root`とpull側のミラー削除は変更不要——窓付きローカルは（窓の分だけ）非空であり、窓外のキーは常にmanifestに存在し続けるため、いずれの既存ロジックも誤動作しない
    - トレードオフとして、CI内の`copilot-retro collect`は窓の中しか見なくなる。窓外アーカイブへの訂正の再取り込みは運用者のローカル全件`pull → collect → push`（`--reports-append-only`を付けない）が担う
