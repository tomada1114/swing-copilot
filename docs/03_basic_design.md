...
- ファンダメンタルズ更新: 週1回、かつ前回取得以降に新規filingがある銘柄のみを対象とする増分更新とする。具体的には、(1) 取得記録が無い（初回）、(2) 前回取得日から7日以上経過、(3) 収集済みfilingメタデータ上で前回取得日以降に提出されたfilingがある——のいずれかを満たす銘柄だけEDGARへ問い合わせる（**Issue #258で実装**。それまでは同日再実行の重複回避しか行っておらず、本規定と乖離していた）。前回取得日時は`fundamentals_fetch_log`に持つ**メタデータ**であり`as_of`の代替ではない。新規filingの判定にはステップ5が既に収集済みの`text_items`（`source_type='filing'`）を使うので、**外部API呼び出しは増えない**。判定の詳細・境界・被覆の限界は`docs/04_detailed_design.md` 3.21節を参照。
