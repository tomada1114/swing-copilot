...
スコープ外（別Issueで扱う）: SECの429（`edgar.httprequests.TooManyRequestsError`）はstaminaの`RETRYABLE_EXCEPTIONS`にも`retry.py`の`_EXTERNAL_FAILURES`にも掛からず両層ともリトライしない——本修正の前後で挙動は変わらない。`stream_with_retry`は`@with_identity`が同期ジェネレータをラップするため`inspect.isgeneratorfunction()`が`False`になり、stamina側の再実行経路に入らず実質リトライされない（edgartools側の潜在バグ、実測attempts=1）。`EdgarClient._throttle()`の`ratelimit.MinIntervalThrottle`への統合、およびedgartools自身の9 req/sスロットルとの二重管理解消も本Issueでは扱わない。
