...
**Issue #447実装時追記（429がどのリトライ層でもリトライされていなかった問題）**: 上記のとおりedgartoolsは429を`edgar.httprequests.TooManyRequestsError`として送出するが、これはstaminaの`RETRYABLE_EXCEPTIONS`にも`retry.py`の（当時の）内部例外集合にも含まれない素の`Exception`サブクラスで、`_with_retries()`の`except`節がそもそも捕捉しないため、`is_retryable`述語をどう書き換えても効かなかった（実測: 1リクエスト、`sleep_fn == []`で即座に伝播）。対処は`retry_external_call`にキーワード専用引数`retryable_types: tuple[type[Exception], ...]`（既定値は従来の共有例外集合`retry.EXTERNAL_FAILURES`）を追加し、捕捉する例外型そのものを呼び出し側から差し替え可能にしたこと。`EdgarClient._with_retries()`は`retryable_types=(*EXTERNAL_FAILURES, TooManyRequestsError)`と、それに対応する述語（`TooManyRequestsError`を無条件でretryable扱いし、それ以外は既存の`is_retryable_external_error`へ委譲）を渡す。`retry.py`自体はedgartoolsをimportしない——他アダプタ（yfinance/Finnhub/FRED）は`retryable_types`を渡さず既定値のままなので挙動は変わらない。回帰テストは`tests/data/test_edgar_http_boundary.py::TestTooManyRequestsIsRetryable`で、429応答が3試行・`sleep_fn == [1.0, 2.0]`になることを固定する。
