...
- 初期逆指値はシグナル日終値をアンカーに、`initial_stop_price(close, atr14, exit_atr_multiple)`（既定は終値−2.5×ATR14）で算出し、本番・仮想台帳・バックテストでこの基準を共有する。バックテストの株数は最悪ケースの`limit_price`を基準にサイジングする一方、現金・手数料・損益の約定単価は実際の約定価格を使う。寄付が有効ストップ以下へギャップした日は寄付で、日中安値だけがストップへ到達した日はストップ価格で約定し、約定日にギャップダウンしても同じ日次ループ内で`days_held=0`のstop決済になる。**移行前後の実測**（`reports/backtests/2026-08-23-issue-341-entry-stop-anchor.md`）: 移行前は約定価格アンカーだったため`k`を1.5/2.0まで上げても`k=0.0`とバイト単位で同一の結果になる回帰があった（このサンプルでは`open ≤ limit`が常に成立し、`next_open`互換アームと同じ約定価格に潰れていたため）。移行後は`k`の増加とともに`avg_invested_pct`が単調に縮小し（19.61%→16.58%）、`avg_r_multiple`も全`k`で改善しており、意図どおり`k`の選択が損益指標へ反映されるようになった。
