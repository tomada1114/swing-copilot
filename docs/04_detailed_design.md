...
**依存**: `edgartools`
**エラー処理**: EDGAR境界の接続・タイムアウト・HTTP 408/5xxは合計3試行（既定backoff 1秒、2秒）までとし、各試行前に10リクエスト/秒制限を適用する。HTTP 429（SECのfair-access制限超過）だけはこの既定backoffに乗らない例外で、待機時間はSEC自身のブロック解除待ち（下記Issue #447実装時追記）に従う。その他の4xx、設定・入力検証エラーはretryしない。fake clock/sleepで「一時失敗後成功」「3回で停止」「全試行がthrottle対象」を検証する。銘柄単位で取得失敗した場合はスキップしログ記録、バッチは継続する。

**Issue #429実装時追記（3試行契約がedgartools内部リトライで無効化されていた問題）**: 上記の「合計3試行」は`EdgarClient._with_retries()`（`retry_external_call`経由）の話であり、実装当初からその内側で呼ぶ`Company.get_facts()`/`get_filings()`/`filing.text()`はedgartoolsライブラリ自身の実装だった。edgartoolsの同期リクエスト関数（`edgar.httprequests`の`get_with_retry`/`stream_with_retry`/`post_with_retry`）はいずれも`stamina`ライブラリの`@retry(...)`デコレータで独自にラップされており、実`time.sleep`・注入不能という条件で、`_with_retries`の**内側**に隠れたリトライループとして動いていた。transport例外（`ConnectError`/`ReadTimeout`等）が続く場合、外側3試行それぞれの内側でedgartoolsが最大`QUICK_RETRY_ATTEMPTS = 5`回再送するため、論理1呼び出しあたり最大15リクエスト・実時間48秒（`tests/data/test_edgar_http_boundary.py`の再現シナリオで実測、`sleep_fn`には未注入）になり得た。`tests/data/test_edgar.py`が`company_factory`偽装（`EdgarClient`の宣言済みシームより下）でこの二重リトライを一度も観測していなかったのが原因である。

対処は`EdgarClient.__init__`で`edgar.set_identity(identity)`の直後に`stamina.set_active(False)`を呼ぶこと（`_disable_edgartools_internal_retries()`）。`stamina.set_active()`はstaminaが公開・文書化した正規のキルスイッチで、呼び出しごとに参照されるため即座に効く。無効化後は`reraise=True`のまま1試行で元の例外がそのまま外へ出るので、`retry_external_call`が受け取る例外型は変わらない。この切替はプロセス全体に効く（stamina側にスレッドローカル／呼び出し単位の変種は無い）が、このvenvで`stamina`をimportするのは`edgartools`のみ、`import edgar`を持つのは`data/edgar.py`だけなので影響範囲は閉じている。

却下した代替案: (a) `QUICK_RETRY_ATTEMPTS`等の定数をモンキーパッチする案は動かない——`@stamina.retry(attempts=...)`はデコレータ引数としてimport時に評価され、値を焼き付けるため、後から定数を書き換えても既存デコレートには効かない（実測確認）。(b) EDGAR側に環境変数でのリトライ無効化オプションは存在しない。(c) 自前HTTPクライアントへの置換は、XBRL正規化・SGMLパース・10-Q章抽出等の非HTTP依存部分を保ったままHTTP層だけ差し替える公開の注入点がedgartoolsに無く、費用対効果が見合わない。

回帰テストは`tests/data/test_edgar_http_boundary.py`（新規）で、`edgar.httprequests.http_client`（edgartoolsの同期リクエスト関数がクライアントを得る唯一の関数）を`httpx.MockTransport`で差し替え、edgartools自身の`@stamina.retry`デコレータを実際に通過させたままリクエスト数と`sleep_fn`への注入値を数える。これは`writing-external-adapters`が定める「アダプタ自身の宣言済みシーム（`company_factory`）を叩け、2層下を叩くな」の意図的な例外であり、機構名（`stamina`）ではなくリクエスト数を数えるのは、将来edgartoolsがリトライ実装を差し替えても検知できるようにするため。`_with_retries()`を通らない経路（`filing.obj()`等）は`tests/data/test_edgar.py::TestEdgartoolsInternalRetry`が構築時に`stamina.is_active()`が`False`になることを直接検査して覆う。

あわせて、`_extract_ten_q_sections()`内の`filing.obj()`だけが`_with_retries()`を通っていなかったのを修正した。`filing.obj()`は`filing.text()`が取得済みのHTMLを再パースするだけとは限らず、提出SGMLにインラインHTMLが無い場合はfiling homepageと主文書をedgartoolsが取りに行くため、EDGAR境界呼び出しである。内部リトライを無効化した状態でここを素通しにすると、transportの一時失敗1回がそのままfail-softハンドラに落ち、10-Qのセクションが静かに`head_fallback`へ劣化する（内部リトライがこの取りこぼしを覆い隠していた）。

スコープ外（別Issueで扱う）: `stream_with_retry`は`@with_identity`が同期ジェネレータをラップするため`inspect.isgeneratorfunction()`が`False`になり、stamina側の再実行経路に入らず実質リトライされない（edgartools側の潜在バグ、実測attempts=1）。`EdgarClient._throttle()`の`ratelimit.MinIntervalThrottle`への統合、およびedgartools自身の9 req/sスロットルとの二重管理解消も本Issueでは扱わない。SECの429（`edgar.httprequests.TooManyRequestsError`）がどちらのリトライ層にも掛からない件は、以下のIssue #447実装時追記のとおり別Issueで解消済み。

**Issue #447実装時追記（429がどのリトライ層でもリトライされていなかった問題、および429専用の待機ルール）**: 上記のとおりedgartoolsは429を`edgar.httprequests.TooManyRequestsError`として送出するが、これはstaminaの`RETRYABLE_EXCEPTIONS`にも`retry.py`の（当時の）内部例外集合にも含まれない素の`Exception`サブクラスで、`_with_retries()`の`except`節がそもそも捕捉しないため、`is_retryable`述語をどう書き換えても効かなかった（実測: 1リクエスト、`sleep_fn == []`で即座に伝播）。

加えて、`TooManyRequestsError`のdocstringはSECが429超過を検知したIPを**約10分間ブロックし、ブロック中に再送するとブロック時間が延長される**と明言している（`Do NOT retry immediately`）。したがって429は単に捕捉して再試行するだけでは不十分で、待機時間そのものを既定の1秒・2秒backoffから差し替える必要がある。

対処は`retry_external_call`にキーワード専用引数`policy: RetryPolicy`（`is_retryable`・`retryable_types`・待機時間を決める`delay_for`の3つをまとめたfrozen dataclass、既定値は従来どおりの挙動——引数個数を増やさずに済むのと、3つがひとつの単位で差し替わることを型で示すため）を追加したこと。`EdgarClient._with_retries()`は`_EDGAR_RETRY_POLICY = RetryPolicy(is_retryable=_is_edgar_error_retryable, retryable_types=(*EXTERNAL_FAILURES, TooManyRequestsError), delay_for=_edgar_retry_delay)`を渡す。`_edgar_retry_delay(error, default)`は`TooManyRequestsError`について、レスポンスの`Retry-After`ヘッダから得た`error.retry_after`（秒）があればそれを、無い（ヘッダ欠如またはパース不能で`None`）場合、および0以下（過去日時のHTTP-dateは`0`、負の整数ヘッダはそのまま通る）の場合は`TooManyRequestsError.BLOCK_DURATION_MINUTES * 60`（600秒）を返し、それ以外の例外は`default`（既定の1秒／2秒）をそのまま返す。`retry.py`自体はedgartoolsをimportしない——他アダプタ（yfinance/Finnhub/FRED）は`policy`を渡さず既定の`RetryPolicy()`のままなので挙動は変わらない。

**ワーストケース**: 合計3試行のうち、1回目・2回目の429がいずれも`Retry-After`無し（=600秒フォールバック）だった場合、2回のブロック待ち（約20分）に3回目の実行時間を加えた分だけEDGAR呼び出し1回がスタールしうる。これはthrottleや接続失敗の待ちよりも桁違いに長く、`copilot-daily`のバッチ全体の実行時間に直接影響しうる既知のトレードオフである——SEC側のブロックを延長しない（＝ブロック解除を待ってから再送する）ことを優先した設計判断であり、待機時間の短縮は本Issueのスコープ外とする。

回帰テストは`tests/data/test_edgar_http_boundary.py::TestTooManyRequestsIsRetryable`で、(a) `Retry-After: 120`付き429が3試行・`sleep_fn == [120.0, 120.0]`、(b) `Retry-After`無し429が3試行・`sleep_fn == [600.0, 600.0]`、(c) パース不能な`Retry-After`付き429、および(d) `Retry-After: 0`／負値付き429も同じく`[600.0, 600.0]`になることを固定する（0秒待機はSEC側のブロックを延長し、負値は本番の実`time.sleep`が`ValueError`を投げるため、どちらも待機値として使えない）。`TestRetryableServerError`は429以外の再試行可能エラー（5xx）が引き続き`[1.0, 2.0]`のままであることを固定する回帰テストとして残す。
...
取引日カレンダーは`pipeline/forward_returns.py`の純関数を3.21aのpostmortemと共有する（逆算`find_target_trading_day`／順算`find_maturity_trading_day`、いずれもベンチマーク銘柄のバー実在日を代替カレンダーとする）。すべて`date <= as_of`の価格のみ使用（look-ahead禁止）。bar欠損は当該`(run, symbol, horizon)`をスキップしてnoteに残すfail-soft、**バーは存在するが終値が非有限（`NaN`/`±inf`）の場合も`compute_forward_return`が`None`を返して同じスキップ扱いにする**（Issue #206。`verdict_outcomes.forward_return_pct`は`DOUBLE NOT NULL`だがDuckDBの`NaN`は`NULL`ではないため、素通しすると「勝ちでも負けでもない行」として永続化され集計を黙って歪める。現状NaN終値を落としているのは`YFinanceProvider`だけで、正規化は各providerの責務という前提上ストア直書き・将来のproviderはこのガードを通らない。回帰テストは`tests/pipeline/test_forward_returns.py::TestComputeForwardReturn::test_returns_none_when_either_close_is_not_finite`）。**満期済みスライスの一部銘柄だけがこのfail-softに落ちるとき、単なるnoteでは済まない（Issue #424）**: `replace_verdict_outcomes`は`(run_id, horizon_days)`単位の完全置換なので、欠けた銘柄を`outcomes`から単に省くと**その銘柄の既存の記録済み行までスライス置換で道連れに削除される**（MNST 2026-08-10、分割ex-date前日のバーをYahooの再取得が落とした事例で実際に発生し、訂正ではなく消滅になっていた）。`_evaluate_slice`は再計算に失敗した銘柄があるときだけ`get_verdict_outcomes_for_slice`でそのスライスの既存行を読み戻し、`recommendation`が現在のverdictと一致する行はそのまま`outcomes`へ引き継いで残す（verdictが訂正され一致しなくなっていれば保持する根拠が無いため、従来どおり削除する）。`EvaluateSummary.preserved_outcome_count`が今回保持した行数を数える。未満期のスライスはnoteに出さず`pending_slice_count`に数える（バッチでは大半が正当に未満期なので、1件ずつnoteに出すと本当のデータ品質シグナルが埋もれる）。**runごと`verdicts`から消えた場合はこの自己修復が効かない（Issue #448）**: `evaluate_verdicts`は`get_verdicts_in_window`が返す run しか走査せず、その集合は`verdicts`テーブル自身に依存するため、あるrunの再collectが1件もverdictを生まなくなると、そのrunは二度と走査対象に戻らず`verdict_outcomes`側の行が永久に孤児化する（銘柄が1つ落ちるだけなら`expected`集合の不一致で`only_pending`判定がスライスを再評価し、`replace_verdict_outcomes`の全置換が自己修復する——壊れているのは「runの全verdictがゼロになる」経路だけ）。`replace_collected_run`は新しい`verdicts`が空のときに限り、同じトランザクション内で`verdict_outcomes WHERE run_id = ?`も削除する。空でない再collectでは発火しない：無条件に発火させると、Issue #424のcarry-forward（終値を再計算できない銘柄の既存行を保持する）を再collectのたびに踏み潰し、#424の不変条件を静かに破ってしまうため。
...
**`entry_price`基準監査（Issue #427）**: `check`は`check_bars`の混在署名/欠損セッション監査に続けて、`risk_assessments.entry_price`の基準ずれを独立に監査する（`check_entry_prices`）。事実として、凍結された`entry_price`は構成上その run 日の**生バーの終値そのもの**である——`entry_price`の出所は`RiskChecker._assess`（`risk/checks.py`）が`candidate.metrics["close"]`（`ranking_metrics`、`SymbolWindow`の`bar_count - 1`位置の close）から取るもので、これは`MarketStore.read_bars(symbols, start, as_of, as_of)`が`adjust_bars`で分割調整した後の値だが、`adjust_bars`が`date == as_of`の行に効かせられる分割は`as_of < ex_date <= as_of`を満たすものだけであり、そんな分割は存在しない。よって`as_of`当日行の累積factorは恒に`1.0`——近似ではなく恒等式である。この恒等式から2つの帰結が出る。(1) **書き込み時ゲートは恒真検査にしかならない**: 日次runが`risk_assessments`を書く瞬間、`entry_price`はまさにその同日バーの終値から取り出した値であり、その場で両者を突き合わせても値をそれ自身の出所と比べるだけで何も検出できない（#423のAPHが当時のゲートをすり抜けたのも、ゲートが甘かったからではなく、当時のストア自体が#418以前の調整済み・基準混在の汚損バーを持っていて両者が実際に**一致していた**ため）。(2) **したがって#423の汚損は事後にしか存在しない乖離である**: `risk_assessments`の行は不変の履歴として凍結された後、`copilot-backfill rebuild`（#413の修復）が**バーだけを**生値へ置き換えるとそこで初めて乖離が生まれる——これは定義上、保存済み履歴を走査する監査でしか捕まらない。この2点が、issueの検討事項のうち「書き込み時ゲートを入れるか」「監査を新設するか」を実装の帰結として決めた。

判定述語`is_entry_price_basis_mismatch(entry_price, bar_close)`（`tracking/update.py`の`ENTRY_PRICE_BAR_TOLERANCE = 0.005`を使う）は、Issue #423が`_seed_position`に持たせた同日整合性チェックと**1本の関数を共有**する——監査と是正が別々に許容幅を持つと、片方だけ動かしたときに食い違う（#425が`adjustments.py`について明示的に退けたのと同じ誤りの形）。監査は`market_store.read_raw_bars()`で読んだ生バーとだけ突き合わせ、**分割を一切参照しない**（`corporate_actions`も`cumulative_split_factors`も使わない）。これは`_seed_position`が`entry_price /= factor`した上で同じfactorで調整済みのバー終値と比べるのと代数的に等価であり（両辺を同じ`F`で割った比較`|e/F - c/F| > (c/F)·tol`は`|e - c| > c·tol`と恒等）、そのぶん**#425が扱うfactor由来の誤検知の族が原理的に入り込まない**——`check_bars`が`read_bars`ではなく`read_raw_bars`を使うのと同じ理由による。

読み出しは`storage/tracking_records.py::get_frozen_entry_prices(database, symbols)`が担う。`risk_assessments`を`runs`と`run_id`でjoinし`run_date`を得る（`verdicts.as_of`とは構成上等しいが、`verdicts`はjoinしない——verdictの無い行、`dry_run`/`failed`のrunの行も等しく汚損の証跡なので対象に含める）。`risk_assessments`/`runs`のいずれのテーブルも存在しないDB（バーだけのバックフィル用途）では`information_schema.tables`で検出して`()`を返し、例外にしない。`entry_price IS NULL`の行はSQLで除外し、scanned/unresolvedいずれにも数えない。`entry_price`または同日の生バー終値が非有限・0以下、あるいは同日の生バーが store に無い行は、比較不能な**データ品質の欠陥**として`unresolved_rows`に数え、findingにはしない（誤検知を避けるため）。

出力は`check_bars`の書式をそのまま踏襲し、`entry_price: ok（対象 N 行 / M 銘柄、基準ずれなし）`または`entry_price: 対象 N 行 / M 銘柄 / 基準ずれ K 行`に続けて`基準ずれ: SYM YYYY-MM-DD（凍結 P1 / 生バー終値 P2、比 R）`（価格は`:.6f`、比は`:.4f`——比は#423のAPHが「ちょうど2.0000」で即断できた診断的な値であり、分類ロジックには使わない）を1行ずつ、続けて`copilot-track rebuild --symbol <SYMBOL>`での是正を促す1行を添える。`unresolved_rows > 0`のときのみ末尾に件数だけの1行（`entry_price 判定不能: N 行（同日の生バーが store に無い）`）を足す。**形式マーカーがNGのときはこの監査自体を走らせない**（#418以前の調整済みバーに凍結値をぶつけても意味を成さないため）。

**書き込み時ゲート（run を止める／汚損値の書き込みを拒む）は入れない。終了コードも0のまま変えない。** 前段の恒真性の議論に加え、是正経路（`copilot-track rebuild`、PR #426でfail-softに実装済み）が既にあり、監査が見つける行は「放置すると壊れる行」ではなく「次のreplayで自動的に直る行」なので、runを止める根拠が無い。#425が`check`への終了コード追加を明示的にスコープ外にしている以上、ここで導入すればバー監査側の契約まで黙って変えることになる。

`pipeline/backfill.py`が`tracking/update.py`（`is_entry_price_basis_mismatch`/`ENTRY_PRICE_BAR_TOLERANCE`）をimportする——層としてはpipeline > trackingなので合法（`daily_runner`は既にtrack updateを呼ぶ）だが、`backfill`から`tracking`への依存は新しい向きである。循環は無い（`tracking/update.py`はstorage/data/modelsしかimportしない）。

**欠損セッションの検出・記録（Issue #449）**: `rebuild`は置換の過程で供給元が返さなくなった保存済みセッションを`供給元が返さなくなったセッション（rebuild により削除済み）: SYM（日付, ...）`として報告する（`replace_symbol_bars`が実際に消す。3.7節）。`bars`（`copilot-daily`と同じ`write_bars`経路）は同じ状況を`供給元が返さなくなったセッション（既存行は保持）: SYM（日付, ...）`と文言を変えて報告する——既存行は消えていないので「保持」と「削除済み」を混同してはならない。`check`は独立の監査ルールを持つ：`MarketStore.session_coverage()`から導いたセッションカレンダー（`--symbols`の有無にかかわらず常にストア全体から作る）に対し、対象銘柄が上場中のセッションでバーを欠く場合を`欠損セッション: SYM N 件（日付, ...）`として列挙し、何も無ければ`check: ok（対象 N 銘柄、混在署名なし、欠損セッションなし）`。いずれの経路も**終了コードを変えず、検疫もしない**（判断の根拠は3.7節の追記を参照）。銘柄あたり日付は最大5件で、`copilot-backfill`側は`古い順 5 件: ..., ...（全 N 件）`と総数を添え、`daily.py`の`run_steps.detail`側は`(+N)`を付ける（同detailは銘柄も最大10件で、超過分は`'...': ['(+N symbols)']`）。いずれも**古い順**に残す（穴が最初に開いたセッションが追跡の起点であり、「最新」と称して先頭を出すのは誤報である）。
