...
| FR-12 | required step の失敗は後続 step を実行せず failed で終える | 価格取得失敗後も分析入力を出力する | `tests/pipeline/test_daily_runner.py::TestFatalStepFailure::test_price_fetch_failure_marks_run_failed_and_stops` |
...
| NFR-03 | 時間予算超過時もローカル出力を残して縮退終了する | text/export/notify を延々実行し、結果を残さない | `tests/pipeline/test_daily_runner.py::TestTimeoutBudget::test_pre_step_breach_skips_network_steps_but_the_run_still_completes` |
...
| NFR-05 | run は再構成可能な metadata を保存する | strategy または universe identity が監査できない | `tests/pipeline/test_daily_runner.py::TestRunFingerprintAndMetadata::test_run_persists_reconstructable_metadata` |
...
| `analysis/export.py` | 登録簿は読み出し専用で、新規 run は退役フィールドを書かない | 退役フィールドがモデルに残ったまま再びシリアライズされる | `tests/analysis/test_export.py::TestRetiredFieldsAreReadOnly::test_export_never_writes_a_field_retired_from_analysis_input` |
...
## run_date は壁時計から作らない（Issue #372）

2026-08-29 の定時実行は、前日 08-28 分の job が寄付前に遅延発火して
`run_date=2026-08-28` を先食いし、翌 08-29（土）の job から見た最新取引日が
既に `status='success'` で埋まっていたため `PREFLIGHT_ABORT[same_day_rerun]`
で正しく中止しただけだった。原因は `pipeline/daily_runner.py` の `run_date`
解決にある——プリフェッチが空・例外のとき既定値の `deps.clock.today()`（壁
時計）へフォールバックし（AGENTS.md「wall time is metadata, never a
substitute for `as_of`」への違反）、成功時も「取得できた最新の bar」を
無条件に採用していた（「引けたセッション」とは限らない）。
...
`copilot-daily` は終了時の状態（`outcome`/`reason`/`run_id`/`run_date`/
`candidates`/`started_at`/`finished_at`）を `reports/<run_date>/<run_id>/`
...
| 対象 | 不変条件 | 代表的な反例 | 検証 |
| --- | --- | --- | --- |
| `pipeline/daily_runner.py` | `--as-of` 未指定でプリフェッチが空のとき `run_date` が壁時計にならず `PreflightAbort(reason="no_trading_day")` で止まる | 空プリフェッチ時に `run_date` を `deps.clock.today()` のまま残し、まだ引けていない当日で run を開始する | `tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_empty_prefetch_aborts_with_no_trading_day_and_writes_no_run` |
| `pipeline/daily_runner.py` | `--as-of` 未指定でプリフェッチが例外を送出したとき `PreflightAbort(reason="price_fetch_failed")` で止まり、`no_trading_day` とは区別される | プリフェッチ例外を `no_trading_day` に丸め、データ取得障害を「取引日が無い」という正常終了として見逃す | `tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_prefetch_exception_aborts_with_price_fetch_failed_and_writes_no_run` |
| `pipeline/daily_runner.py` | `--as-of` 未指定でプリフェッチが空かつ `failures` を伴うとき `PreflightAbort(reason="price_fetch_failed")` で止まる（`YFinanceProvider` は例外を送出せず、障害を「空フレーム＋銘柄ごとの `FetchFailure`」に畳んで返すため、本番の取得障害はこの形で到着する） | 失敗を伴う空応答を `no_trading_day` に分類し、`check_daily_complete.py` の正当停止ホワイトリストに載せて、プロバイダ障害の日を「分析なしで緑」にする | `tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_empty_prefetch_with_failures_is_price_fetch_failed_not_no_trading_day` |
| `pipeline/daily_runner.py` | `run_date` は米国東部時間 16:00 を過ぎて引けたセッションの最新日であり、境界は 16:00 ET を含む | 最新 bar が当日かつ 16:00 ET 前でもその日を `run_date` として採用する | `tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_latest_bar_still_mid_session_falls_back_to_the_prior_close`、`tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_latest_bar_exactly_at_the_close_boundary_is_used`、`tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_latest_bar_just_after_the_close_is_used` |
| `pipeline/daily_runner.py` | 土曜の壁時計でも、最新の引けたセッションが金曜なら `run_date` は金曜になる | 壁時計の「今日」（土曜）をそのまま `run_date` にしてしまう | `tests/pipeline/test_daily_runner.py::TestRunDateResolvesOnlyClosedSessions::test_saturday_wall_clock_with_fridays_bar_resolves_to_friday` |
| `pipeline/daily_runner.py` | `--as-of` 明示時は閉じたセッション判定を経由せず、指定日をそのまま採用する | `--as-of` でもプリフェッチ・16:00 ET 判定を通し、壁時計次第で結果が変わる | `tests/pipeline/test_daily_runner.py::TestAsOfDefaulting::test_explicit_as_of_bypasses_the_closed_session_check` |
