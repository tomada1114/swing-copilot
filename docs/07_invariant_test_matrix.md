...
| `pipeline/daily_runner.py` | `--as-of` 未指定でプリフェッチが空かつ `failures` を伴うとき `PreflightAbort(reason="price_fetch_failed")` で止まる（`YFinanceProvider` は例外を送出せず、障害を「空フレーム＋銘柄ごとの `FetchFailure`」に畳んで返すため、本番の取得障害はこの形で到着する） | 失敗を伴う空応答を `no_trading_day` に分類し、`check_daily_complete.py` の正当停止ホワイトリストに載せて、プロバイダ障害の日を「分析なしで緑」にする | `tests/pipeline/test_daily_core.py::TestRunDateResolvesOnlyClosedSessions::test_empty_prefetch_with_failures_is_price_fetch_failed_not_no_trading_day` |
