...
- regime ゲート・決算近接ブロック・銘柄単位の価格計画は**本番経路に実装済み**
  （`risk/checks.py`）。口座依存のportfolio heatは#348で本番経路から撤去した。
  06 ロードマップ D3/D7の履歴記述はその時点の状態として読む。残る検討事項は、
  本番とバックテストのシミュレーション規約を明示的に分けて検証すること（#349）。
...
3. **点推定だけで設定を変えない**: 散らばり（標準誤差・信頼区間・実効サンプル数）
   ...
   - #252のSMA200/FTD/Exposure再設計は、`dd-forward`の点推定で閾値を調整したもの
     ではない。標準的な実務とオーナー判断を根拠にし、既存のDD 7/6は維持する。
