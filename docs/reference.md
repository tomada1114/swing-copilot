...
も再書き込みもしない（Issue #209）。出力は
「走査 / 解析 / 無変更 / 解析不能 / 取り込み」のrun数を並べて表示する。
...
にそのまま列として出る。満期済みスライスのうち一部銘柄だけ終値を欠くときは例外で、
単に省くと`(run_id, horizon_days)`単位の完全置換がその銘柄の既存記録行まで
スライスごと削除してしまう（Issue #424、MNST 2026-08-10の再発事例）。そのときだけ
`get_verdict_outcomes_for_slice`で既存行を読み戻し、`recommendation`が一致する限り
`outcomes`へそのまま引き継いで残す（`EvaluateSummary.preserved_outcome_count`）。
`--db`の兄弟`bars/`がディレクトリごと無い場合、`evaluate`
...
`隔離した銘柄: ...`として同じ出力に加わる（既存行は不変）。供給元が
過去のセッションを返さなくなった場合（Issue #449、下記「欠損セッション
の検出・記録」節参照）は`供給元が返さなくなったセッション（既存行は
保持）: SYM（日付, ...）`が加わる——`write_bars`は行を消さないので、
既存行は保持されたまま報告だけが増える。
...
供給元が過去のセッションを返さなくなった場合は`供給元が返さなくなった
セッション（rebuild により削除済み）: SYM（日付, ...）`が加わる——
`replace_symbol_bars`は実際に既存行を全て消してから書き直すので、
`bars`側の文言（既存行は保持）とは意図的に書き分けている。
...
`copilot-backfill rebuild`が全履歴を再取得する際、供給元（Yahoo）が
過去に返していたバーを黙って返さなくなることがある（MNST
2026-08-10——分割ex-date前日のバーが再取得のたびに消えていた）。
0.5%訂正ゲート・混在署名ゲートはいずれも「値が変わった行」を検疫する
防御であり、「行そのものが消えた」ケースは対象外だったため、これを
**検出・記録**する（検疫はしない——`write_bars`は行を消さないので、
日次経路・`bars`では守るべき資産を何も失わない。詳細は
`docs/04_detailed_design.md` 3.7節）。

- `bars`/`copilot-daily`: 供給元が返さなくなったセッションを報告するが
  既存行は保持されたまま。
- `rebuild`: 全履歴を置き換えるので、報告されたセッションは実際に消える。
- `check`: `MarketStore.session_coverage()`から独自のセッションカレンダー
  を導出し（`--symbols`の有無にかかわらず常にストア全体から作る）、
  対象銘柄が上場中のセッションでバーを欠く場合を欠損として報告する。
  非株式カレンダー銘柄`^TNX`は`--symbols`省略時の走査対象からのみ除外し
  （明示すれば走査する）、それ以外のエッジケース（上場廃止・休場日・
  上場前・単発の誤バー）はセッション判定の閾値`_SESSION_QUORUM_RATIO
  = 0.5`（銘柄件数に対するその日のバー保有件数の比）1つで吸収する。
  この閾値は実データで検証されていない。

いずれの経路も終了コードを変えず、`PREFLIGHT_ABORT`の語彙も増やさない。
銘柄あたり最大5件を超える日付は要約する——`copilot-backfill`の各行は
古い順に5件だけ並べて`古い順 5 件: ..., ...（全 N 件）`と総数を添え、
`copilot-daily`の`run_steps.detail`は同じ先頭5件に`(+N)`を付ける
（銘柄も最大10件で、超過分は`'...': ['(+N symbols)']`）。いずれも
**古い方**を残す——穴が最初に開いたセッションが追跡の起点だからである。
