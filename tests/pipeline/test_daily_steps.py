"""Tests for pipeline/daily.py's step behavior (FR-12).

Covers what each of the fatal steps 1-4 itself does: price/corporate-action
storage, the fundamentals freshness/incremental-refresh rule and its
held-first fetch order, screening rejections/truncations, and the
`--limit`/universe symbol-selection helper that feeds every step's symbol
list. Lifecycle behavior -- fatal short-circuit, the NFR-03 timeout budget,
preflight-adjacent checks, rerun guards, and run-date resolution -- is
covered by tests/pipeline/test_daily_runner.py. Fail-soft steps 5-9 are
covered by tests/pipeline/test_failsoft.py and tests/test_e2e_smoke.py.
"""
